...
def read():
    threading.Timer(60.0, read).start()
    t = datetime.date.today()

    # Maybe need to follow system datetime format
    today = t.strftime('%#d/%#m/%Y')        # 23/9/2022
    #today = t.strftime('%#m/%#d/%Y')        # 9/23/2022
    ...
    for row in cursor.fetchall():
        ...
        if next_read == formatted_now:
            ...
            cursor.execute("UPDATE machine_db SET datetime_processed = " + "'" + formatted_now + "'" +  ", next_read = " + "'" + formatted_new_next_read + "'" + " WHERE id = " + str(id))
            connection.commit()
